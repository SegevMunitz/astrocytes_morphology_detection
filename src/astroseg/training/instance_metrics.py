...
def _validate_instances(labels: np.ndarray, name: str) -> np.ndarray:
    ...
    Labels need not be sequential, but zero is always interpreted as background.
    Exact integer values are required so interpolation artifacts fail visibly.
    """
...
def match_instances(
    ...
    Hungarian matching prevents one fragmented or merged object from receiving
    credit for several cells. All matches are returned; callers apply thresholds.
    """
...
def instance_segmentation_metrics(
    ...
    Precision, recall, F1, mean matched IoU, and panoptic quality are reported at
    one explicit IoU threshold. Empty prediction/target cases remain finite.
    """
...
def process_ownership_accuracy(
    ...
    Target cells are mapped one-to-one to predictions by IoU. A process pixel is
    correct only when its predicted ID is the matched owner; missed cells score zero.
    """
    prediction = _validate_instances(predicted, "predicted")
    truth = _validate_instances(target, "target")
