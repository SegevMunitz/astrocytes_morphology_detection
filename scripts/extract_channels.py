...
from astroseg.visualization import save_gfap_preview
...
def extract_channels(manifest_path: Path, output_directory: Path) -> int:
    """Save channel arrays and PNG previews for every manifest row."""
    ...
    for _, row in manifest.iterrows():
        ...
        save_gfap_preview(dapi, preview_directory / f"{row['image_id']}_dapi.png")
        count += 1
    return count
